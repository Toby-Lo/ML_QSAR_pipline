...
python step35_admet_filter.py \
    --input-parquet ./results/step34_virtual_screening.parquet \
    --admetlab-file ./data/admetlab_results.csv \
    --mode default \
    --include-bbb \
    --make-plots
"""
...
ENDPOINTS: Tuple[EndpointSpec, ...] = (
    EndpointSpec("hia", ("HIA", "HIA+", "Human Intestinal Absorption", "HIA_prob", "HIA (prob)"), "higher_better", "prob"),
    EndpointSpec("caco2", ("Caco2", "Caco-2", "Caco-2 permeability", "Caco2_permeability", "Caco2 (logPapp)"), "higher_better", "continuous"),
    ...
    EndpointSpec("cyp3a4_inh", ("CYP3A4 inhibitor", "CYP3A4 inhibition", "CYP3A4_inhibition", "CYP3A4_inh", "CYP3A4 (inh prob)"), "lower_better", "prob"),
    EndpointSpec("cyp2d6_inh", ("CYP2D6 inhibitor", "CYP2D6 inhibition", "CYP2D6_inhibition", "CYP2D6_inh", "CYP2D6 (inh prob)"), "lower_better", "prob"),
...
def compute_subscores(df: pd.DataFrame, include_bbb: bool, include_dili: bool) -> pd.DataFrame:
    # Endpoint normalized columns are "goodness" scores already.
    hia = df.get("hia_norm", pd.Series(0.5, index=df.index))
    caco2 = df.get("caco2_norm", pd.Series(0.5, index=df.index))
    ppb = df.get("ppb_norm", pd.Series(0.5, index=df.index))
    bbb = df.get("bbb_norm", pd.Series(0.5, index=df.index))
    cyp3a4 = df.get("cyp3a4_inh_norm", pd.Series(0.5, index=df.index))
    cyp2d6 = df.get("cyp2d6_inh_norm", pd.Series(0.5, index=df.index))
    herg = df.get("herg_norm", pd.Series(0.5, index=df.index))
    ames = df.get("ames_norm", pd.Series(0.5, index=df.index))
    dili = df.get("dili_norm", pd.Series(0.5, index=df.index))

    absorption_score = 0.5 * hia + 0.5 * caco2
    if include_bbb:
        distribution_score = 0.8 * ppb + 0.2 * bbb
    else:
        distribution_score = ppb
    metabolism_score = 0.5 * cyp3a4 + 0.5 * cyp2d6
    if include_dili:
        toxicity_score = (herg + ames + dili) / 3.0
    else:
        toxicity_score = 0.5 * herg + 0.5 * ames
...
def compute_admet_score(df: pd.DataFrame, mode: str) -> pd.Series:
    # Baseline weights from the user prompt.
    if mode == "strict":
        weights = {"absorption": 0.20, "distribution": 0.10, "metabolism": 0.25, "toxicity": 0.45}
    elif mode == "lenient":
        weights = {"absorption": 0.30, "distribution": 0.20, "metabolism": 0.25, "toxicity": 0.25}
    else:
        weights = {"absorption": 0.25, "distribution": 0.15, "metabolism": 0.25, "toxicity": 0.35}

    s = (
        weights["absorption"] * df["absorption_score"].astype("float64")
        + weights["distribution"] * df["distribution_score"].astype("float64")
        + weights["metabolism"] * df["metabolism_score"].astype("float64")
        + weights["toxicity"] * df["toxicity_score"].astype("float64")
    )
...
def main() -> None:
    ...
    admet_tbl = admet_tbl.drop_duplicates(subset=["smiles"], keep="first")
    ...
    merged = compute_subscores(merged, include_bbb=bool(args.include_bbb), include_dili=bool(args.include_dili))
    merged["admet_score"] = compute_admet_score(merged, mode=str(args.mode)).astype("float32")
