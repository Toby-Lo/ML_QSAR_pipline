...
def load_md_data(path: Path, dt: float, ntwx: int, stride: int):
    ...
    if data.shape[1] == 1:
        frames = np.arange(1, data.shape[0] + 1, dtype=float)
        x = (frames * ntwx * stride * dt) / 1000.0
        y = data[:, 0]
        return x, [y], ["y"]
    frames = data[:, 0]
    x = (frames * ntwx * stride * dt) / 1000.0
...
def main():
    ...
    ap.add_argument("--ylabel", default="Ligand RMSD (Å)")
    ...
    p = Path(args.inp)
    x, ys, labels = load_md_data(p, args.dt, args.ntwx, args.stride)
