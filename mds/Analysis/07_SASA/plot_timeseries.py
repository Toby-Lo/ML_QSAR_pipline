...
def infer_ylabel(inp: Path) -> str:
    n = inp.name.lower()
    if "protein" in n:
        return "Protein SASA (Å²)"
    if "ligand" in n:
        return "Ligand SASA (Å²)"
    if "total" in n:
        return "Total SASA (Å²)"
    return "SASA (Å²)"
...
def main():
    ...
    ap.add_argument("--ylabel", default=None, help="Override y-axis label. Default auto-infers Protein/Ligand/Total from input filename.")
    ...
    ylabel = args.ylabel if args.ylabel else infer_ylabel(p)
    ...
    # SASA scale is typically large; keep ticks readable.
    major = max(10.0, round(span / 8.0 / 10.0) * 10.0)
    minor = major / 2.0
